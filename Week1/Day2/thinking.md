...
> embedding是静态语义，经过attention后就会变成动态语义存在理解偏差
>> Transformer内部的Embedding Layer(如nn.Embedding)：这一层确实是静态的查找表，每个token对应一个固定向量，不感知上下文。经过多层self-Attention后变成上下文相关的表示。"苹果 = 0.5x苹果公司 + 0.5x水果苹果"这个比喻只适用于这一层（以及 Word2Vec/GloVe 等老一代词向量模型）。
> 
>> Sentence Transformer输出的Embedding（比如bge-small-zh-v1.5模型）：部分模型（如实验使用的）内部已经跑完所有Attention层，输出的是整句的上下文感知向量，**不是静态**的。已经根据上下文做了语义消歧。
...
1. embedding 与 transformer 的关系？
   A：transformer中有Embedding Layer层，主要是根据文字转化为向量
...
在地理距离，数值异常检测中用欧氏
当向量 L2-归一化后（||v|| = 1），有 ||a - b||² = 2(1 - cos(a,b))，所以欧氏距离和余弦相似度是单调等价的，不只是"近似"
...
另外数据库要建立索引，不然检索会很慢，常见索引算法暴力搜索（精准但慢），IVF倒排文件（分桶），HNSW图结构（最常用）和工具库FAISS/Annoy
 
传统数据库和向量数据库的本质区别
- 传统数据库：精准匹配，基于B-Tree/Hash索引
- 向量数据库：近似相近邻（ANN），基于HNSW/IVF等索引，返回的是相似度排序，存在召回率vs性能的trade-off

### 实验

### 结论
embedding 是核心，决定语义空间质量，embedding好（清洗，切割）->检索准
...
### 理解
"知其然知其所以然"，光知道embedding将词/句向量化并通过余弦相似度找到相近语义不够，要知道是不是任何词，任何语义都能准确识别。中华文化博大精深，~~一词多义是很常见的，对于大部分embedding模型（我猜的），这一点都做得不好~~，老一代的embedding算法都是平均语义，且是静态的，不会根据上下文动态变化，而现代的embedding有sentence transformer，是句型模型，内部有Attention处理，对一词多义的处理比想象中好很多。~~也就容易出现词相同，语义不同，却有较高的相似度~~。除此之外，长文本embedding后容易丢失细节，一些关键信息被淡化了。还有一个容易忽视的就是否定句，以及双重否定表肯定这种，部分embedding模型对这类也处理的不好。
